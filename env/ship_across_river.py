...
class ShipAcrossRiver(core.Env):
    def __init__(self, state_normalize=True, isContinus=False):
        self.type = type
        # 设置动作空间
        self.min_acc_velocity, self.max_acc_velocity = -2, 5
        self.min_angle_velocity, self.max_angle_velocity = -1, 1
        self.isContinus = isContinus
        ...
        if self.isContinus:
            ...
            self.action_space = spaces.Box(np.float32(action_space_low), np.float32(action_space_high))
        else:
        ...
        self.min_velocity, self.max_velocity = 1, 4
        self.min_angle_velocity, self.max_angle_velocity = -1, 1
        ...
        self.observation_space = spaces.Box(np.float32(observation_space_low), np.float32(observation_space_high))
        ...
        self.sub_optimal_1_y, self.sub_optimal_1_range = 50, 10
        self.sub_optimal_2_y, self.sub_optimal_2_range = 80, 20
        # print(3 * (self.optimal_range - 0))
        # print((self.sub_optimal_1_range - 0))
        # print((self.sub_optimal_2_range - 0) * 0.5)
    ...
    def _set_max_water_speed(self):
        ...
        :return:
        '''
        self.max_water_speed = 4 + 1.0 * np.random.standard_normal()
    ...
    def reset_rand(self):
        ...
        # 设置小船初始状态
        self.x = 50 * random.random()
        self.y = 100 * random.random()
        self.angle = -np.pi / 3 + random.random() * 2 * np.pi / 3
        self.velocity = 1 + random.random() * 3
        self.angle_velocity = -1 + random.random() * 2
        return self._get_state()

    def reset_half_rand(self):
        # 设置水流速度
        self._set_max_water_speed()
    ...
    def step(self, action):
        ...
        # self.min_angle_velocity, self.max_angle_velocity = -1, 1
        if self.isContinus:
            acc_velocity = self.min_acc_velocity + action[0] * (self.max_acc_velocity - self.min_acc_velocity)
            acc_angle_velocity = self.min_angle_velocity + \
                                 action[1] * (self.max_angle_velocity - self.min_angle_velocity)
        else:
            acc_velocity = self.min_acc_velocity + action[0] * (self.max_acc_velocity - self.min_acc_velocity) / 10
            acc_angle_velocity = self.min_angle_velocity + \
                                 action[1] * (self.max_angle_velocity - self.min_angle_velocity) / 10

        """
        print("acc_velocity:", acc_velocity)
        print("acc_angle_velocity:", acc_angle_velocity)
        """
        ...
        # print(new_x - old_x)
        # 计算reward
        reward, info = self._cal_reward()
        if info == -1: # info used only for nui-ddqn
            done = False
        else:
            done = True
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            sub_optimal_goal_center.set_color(1, 0, 0.5)
            ...
            sub_optimal_goal_center2.set_color(1, 0, 0.5)
...
def test_env():
    Continus = True
    env = ShipAcrossRiver(isContinus=Continus)
    agent1 = lambda ob: env.action_space.sample()
    agent2 = lambda ob: env.action_space.sample()
    ob = env.reset()
    len = 0
    for _ in range(100):
        if Continus:
